...
/** 在庫マスターを取得（売却済は除外、予約済は reserved=true で表示）。失敗時は空配列。 */
export async function fetchInventory(): Promise<InventoryItem[]> {
  try {
    ...
    return rows
      ...
      .filter((it) => it.stockId && it.title && !SOLD.test(it.status))
      .map(({ status, ...rest }) => {
        void status;
        return rest;
      });
